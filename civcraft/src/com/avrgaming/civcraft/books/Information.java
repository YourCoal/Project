package com.avrgaming.civcraft.books;
...
public class Information {
	...
	public static Inventory techInventory = null;
	...
	public static void showTechInventory(Player player) {	
		if (techInventory == null) {
			techInventory = Bukkit.getServer().createInventory(player, 9*3, "Technology Tree");
			
			techInventory.setItem(0, LoreGuiItem.build(CivColor.LightBlue+ChatColor.BOLD+"What are Technologies?", ItemManager.getId(Material.PAPER), 0, 
				ChatColor.RESET+"Technologies are things that a civilization can,",
				ChatColor.RESET+"research in order to unlock many things like",
				ChatColor.RESET+"armor, weapons, items, structures, civics, and",
				ChatColor.RESET+"even more."
				));
			
			techInventory.addItem(LoreGuiItem.build(CivColor.LightBlue+ChatColor.BOLD+"Technology Tree", ItemManager.getId(Material.COMPASS), 0, 
			...
			techInventory.addItem(LoreGuiItem.build(CivColor.LightBlue+ChatColor.BOLD+"Resources and Materials", ItemManager.getId(Material.DIAMOND_ORE), 0, 
			...
			techInventory.addItem(LoreGuiItem.build(CivColor.LightBlue+ChatColor.BOLD+"Towns", ItemManager.getId(Material.FENCE), 0, 
			...
			techInventory.addItem(LoreGuiItem.build(CivColor.LightBlue+ChatColor.BOLD+"Civilizations", ItemManager.getId(Material.GOLD_HELMET), 0, 
			...
			if (CivGlobal.isCasualMode()) {
				techInventory.addItem(LoreGuiItem.build(CivColor.LightBlue+ChatColor.BOLD+"Casual War!", ItemManager.getId(Material.FIREWORK), 0, 
				...
				techInventory.addItem(LoreGuiItem.build(CivColor.LightBlue+ChatColor.BOLD+"War!", ItemManager.getId(Material.IRON_SWORD), 0, 
						ChatColor.RESET+"War allows civilizations to settle their differences.",
			...
			techInventory.setItem(8, LoreGuiItem.build(CivColor.LightBlue+ChatColor.BOLD+"More Info?", ItemManager.getId(Material.BOOK_AND_QUILL), 0, 
			...
			techInventory.setItem(9, LoreGuiItem.build(CivColor.LightBlue+ChatColor.BOLD+"QUEST: Build a Camp", ItemManager.getId(Material.BOOK_AND_QUILL), 0, 
			...
			techInventory.setItem(18,getInfoBookForItem("civ:found_camp"));
			
			techInventory.setItem(10, LoreGuiItem.build(CivColor.LightBlue+ChatColor.BOLD+"QUEST: Found a Civ", ItemManager.getId(Material.BOOK_AND_QUILL), 0, 
			...
			techInventory.setItem(19,getInfoBookForItem("civ:found_civ"));
			
			techInventory.setItem(11, LoreGuiItem.build(CivColor.LightBlue+ChatColor.BOLD+"Need to know a recipe?", ItemManager.getId(Material.WORKBENCH), 0, 
			...
			LoreGuiItemListener.guiInventories.put(techInventory.getName(), techInventory);
		}
		
		if (player != null && player.isOnline() && player.isValid()) {
			player.openInventory(techInventory);	
		}
	}
...
//				//Cancel out recipes in this category, we don't want people seeing them
//				if (ConfigMaterialCategory.getCategory("Hidden") != null) {
//					inv.clear();
//				}
	...
	public static void spawnGuiBook(Player player) {
		if (guiInventory == null) {
			...
			ItemStack infoRec = LoreGuiItem.build("CivCraft Info", ItemManager.getId(Material.WRITTEN_BOOK), 
			...
			ItemStack craftRec = LoreGuiItem.build("Crafting Recipes", ItemManager.getId(Material.WRITTEN_BOOK), 
					0, CivColor.Gold+"<Click To View>");
			...
			ItemStack techRec = LoreGuiItem.build("Tech Info", ItemManager.getId(Material.POTION), 
					8198, CivColor.Gold+"<Click To View>");
			techRec = LoreGuiItem.setAction(techRec, "OpenInventory");
			techRec = LoreGuiItem.setActionData(techRec, "invType", "showtechInventory");
			guiInventory.addItem(techRec);
			
			ItemStack buildMenu = LoreGuiItem.build("Build Structure", ItemManager.getId(Material.BRICK_STAIRS),
					0, CivColor.Gold+"<Click to View>");
			buildMenu = LoreGuiItem.setAction(buildMenu, "BuildStructureList");
			guiInventory.addItem(buildMenu);
			
			ItemStack embassyMenu = LoreGuiItem.build("Teleport to an Embassy", ItemManager.getId(Material.ENDER_PEARL),
					0, CivColor.Gold+"<Click to View>");
			embassyMenu = LoreGuiItem.setAction(embassyMenu, "TeleportList");
			guiInventory.addItem(embassyMenu);
