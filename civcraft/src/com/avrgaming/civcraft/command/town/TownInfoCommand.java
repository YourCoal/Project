...
public class TownInfoCommand extends CommandBase {
	...
	public void init() {
		...
		commands.put("science", "Shows information about this town's science");
	...
	public void area_cmd() throws CivException {
		...
		CivMessage.send(sender, CivColor.Green+" Happiness:"+CivColor.LightGreen+df.format(happiness)+
				...
				CivColor.Green+" Science:"+CivColor.LightGreen+df.format(science));
	...
	public void science_cmd() throws CivException {
		...
		CivMessage.sendHeading(sender, "Science Info");
		
		AttrSource scienceSources = town.getScience();
		CivMessage.send(sender, scienceSources.getSourceDisplayString(CivColor.Green, CivColor.LightGreen));
		CivMessage.send(sender, scienceSources.getRateDisplayString(CivColor.Green, CivColor.LightGreen));
		CivMessage.send(sender, scienceSources.getTotalDisplayString(CivColor.Green, CivColor.LightGreen));
	...
	public void rates_cmd() throws CivException {
		...
		CivMessage.send(sender, 
				...
				CivColor.Green+" Science: "+CivColor.LightGreen+(town.getScienceRate().total*100)			
				);
	...
	public void lab_cmd() throws CivException {
		...
		for (Structure struct : town.getStructures()) {
			...
			out.add(CivColor.Green+" Last Result: "+CivColor.Yellow+lab.getLastResult().name());
			total +=lab.getBonusScience();
		}
		out.add(CivColor.Green+"----------------------------");
		out.add(CivColor.Green+"Sub Total: "+CivColor.Yellow+total);
		out.add(CivColor.Green+"Total: "+CivColor.Yellow+df.format(total)+" science (estimate).");
		CivMessage.send(sender, out);
	}
	...
	public void monument_cmd() throws CivException {
		Town town = getSelectedTown();
		ArrayList<String> out = new ArrayList<String>();	
		CivMessage.sendHeading(sender, town.getName()+" Monument Info");
		double total = 0;
		for (Structure struct : town.getStructures()) {
			if (!struct.getConfigId().equals("ti_monument")) {
				continue;
			}
			...
			out.add(color+"Monument ("+struct.getCorner()+")");
			out.add(CivColor.Green+"    Level: "+CivColor.Yellow+monument.getLevel()+
					CivColor.Green+" Count: "+CivColor.Yellow+"("+monument.getCount()+"/"+monument.getMaxCount()+")");
			out.add(CivColor.Green+" Last Result: "+CivColor.Yellow+monument.getLastResult().name());
			total += monument.getBonusCulture();
		}
		out.add(CivColor.Green+"----------------------------");
		out.add(CivColor.Green+"Sub Total: "+CivColor.Yellow+total);
		out.add(CivColor.Green+"Total: "+CivColor.Yellow+df.format(total)+" culture (estimate).");
		CivMessage.send(sender, out);
	}
	...
	public static void show(CommandSender sender, Resident resident, Town town, Civilization civ, CommandBase parent) throws CivException {
		...
		if (resident == null || civ.hasResident(resident) || isAdmin) {
			...
			if (town.getTileCount() > level.tiles) { color = CivColor.Rose; }
			if (town.getOutpostCount() > level.outposts) { color = CivColor.Rose; }
			if (town.getStrategicCount() > level.strategics) { color = CivColor.Rose; }
			
			CivMessage.send(sender, CivColor.Green+"Town Level: "+CivColor.LightGreen+town.getLevel()+" ("+town.getLevelTitle()+")"+
					CivColor.Green+" | Plots: "+CivColor.LightGreen+"("+town.getTownChunks().size()+"/"+town.getMaxPlots()+") ");
			
			CivMessage.send(sender, CivColor.Green+"Tiles: "+CivColor.LightGreen+"("+color+town.getTileCount()+CivColor.LightGreen+"/"+level.tiles+")"
			...
			CivMessage.send(sender, CivColor.Green+"Growth: "+CivColor.LightGreen+df.format(town.getGrowth().total)+
									...
									CivColor.Green+" | Science: "+CivColor.LightGreen+df.format(town.getScience().total));
			...
			for (BonusGoodie goodie : town.getBonusGoodies()) {
				if (town.getBonusGoodies().size() > 0) {
					...
					goodies += CivColor.LightGreenItalic+"- None -";
				}
			}
			...
			CivMessage.send(sender, CivColor.Green+"Culture: "+CivColor.LightGreen+clc.level+" ("+town.getAccumulatedCulture()+"/"+clc.amount+")"
									+CivColor.Green+" | Goodies: "+CivColor.LightGreen+goodies);
...
//			CivMessage.send(sender, CivColor.Green+"Faith Level: "+CivColor.LightGreen+cfl.level+" ("+town.getAccumulatedFaith()+"/"+cfl.amount+")");
		...
		if (town.inDebt()) {
			...
			CivMessage.send(sender, CivColor.Yellow+"Use '/town deposit (amount)' to pay it off.");
		}
