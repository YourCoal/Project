...
public class ScienceTimer extends CivAsyncTask {

	//private double beakersPerRun;
	
	public static final int BEAKER_PERIOD = 60;
	
	public ScienceTimer(int periodInSeconds) {
	...
	public void run() {
		for (Civilization civ : CivGlobal.getCivs()) {
			...
			if (townhall == null) {
				CivMessage.sendCiv(civ, "Your captial doesn't have a town hall! You are not generating any science.");
			}
			...
			try {
				...
				if (civ.getResearchTech() != null) {
					civ.addScience(civ.getScience() / BEAKER_PERIOD);
				} else {
					civ.processUnusedScience();
				}
			} catch (Exception e) {
				e.printStackTrace();
			}
