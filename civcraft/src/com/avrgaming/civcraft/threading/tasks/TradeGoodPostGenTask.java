		...
		public void run() {
			try {
				...
				for (int i = 0; i < amount; i++) {
					...
					bcoord2.setWorldname(CivSettings.getString(CivSettings.globalConfig, "worldname"));
					...
					if (ItemManager.getBlockTypeIdAt(world, centerX, centerY-1, centerZ) == CivData.WATER_STILL || 
						...
						if (bcoord2.getCenteredLocation().getBlock().getBiome() != Biome.OCEAN ||
							bcoord2.getCenteredLocation().getBlock().getBiome() != Biome.DEEP_OCEAN ||
							bcoord2.getCenteredLocation().getBlock().getBiome() != Biome.FROZEN_OCEAN) {
							CivLog.warning(" -------------------------------- ");
