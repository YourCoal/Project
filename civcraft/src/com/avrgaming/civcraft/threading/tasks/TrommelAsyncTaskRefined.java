...
public class TrommelAsyncTaskRefined extends CivAsyncTask {
	...
	public static void debug(Trommel trommel, String msg) {
		if (debugTowns.contains(trommel.getTown().getName())) {
			CivLog.warning("Trommel(Refined) Debug:"+trommel.getTown().getName()+":"+msg);
		}
	}	
	
	public TrommelAsyncTaskRefined(Structure trommel) {
		this.trommel = (Trommel)trommel;
	}
	
	public void processTrommelUpdate() {
		if (!trommel.isActive()) {
			debug(trommel, "Trommel(Refined) inactive...");
			return;
		}
		
		debug(trommel, "Processing Trommel(Refined) ...");
		...
		if (sources.size() != 2 || destinations.size() != 2) {
			CivLog.error("Bad chests for Trommel(Refined) in town:"+trommel.getTown().getName()+" sources:"+sources.size()+" dests:"+destinations.size());
			return;
		}
		...
		try {
			for (StructureChest src : sources) {
				...
				try {
					...
					e.printStackTrace();
					CivLog.warning("Trommel(Refined):"+e.getMessage());
					return;
				}
				if (tmp == null) {
