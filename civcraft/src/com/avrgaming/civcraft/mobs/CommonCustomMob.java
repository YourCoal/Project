...
public class CommonCustomMob implements ICustomMob,Listener {
	...
	public static LinkedList<MobDrop> drops = new LinkedList<MobDrop>();
	...
	private static int coinMin = 0;
	private static int coinMax = 0;
	...
	public PathfinderGoalSelector getGoalSelector() {
		...
		Field gsa;
		try {
			gsa = net.minecraft.server.v1_10_R1.EntityInsentient.class.getDeclaredField("goalSelector");
	...
	public PathfinderGoalSelector getTargetSelector() {
		Field gsa;
		try {
			gsa = net.minecraft.server.v1_10_R1.EntityInsentient.class.getDeclaredField("targetSelector");
			gsa.setAccessible(true);
			return (PathfinderGoalSelector)gsa.get((EntityInsentient)entity);
		} catch (NoSuchFieldException e) {
			e.printStackTrace();
	...
	public static Location getLocation(EntityLiving entity2) {
...
//		dropItems();
	...
	public void addVanillaDrop(int type, short data, double chance) {
		...
		CommonCustomMob.drops.add(drop);
	}
	...
	public static LinkedList<MobDrop> getRandomDrops() {
	...
	public void deathEvent(EntityDeathEvent e) {
		if (e.getEntity() instanceof Zombie || e.getEntity()instanceof PigZombie ||
				e.getEntity() instanceof Witch || e.getEntity() instanceof IronGolem) {
			e.getDrops().clear();
			e.setDroppedExp(0);
			
			e.setDroppedExp(this.getEXP());
			this.dropItems(e.getEntity());
			
			customMobs.remove(e.getEntity().getUniqueId());
		}
	}
	...
	public int getEXP() {
		int coins = 0;
		if (coinMax != 0 && coinMin != 0) {
			Random random = new Random();
			coins = random.nextInt(coinMax - coinMin) + coinMin;
		}
		return coins;
	}
	
	public void coinDrop(int min, int max) {
		CommonCustomMob.coinMin = min;
		CommonCustomMob.coinMax = max;
	}
