...
/*	public boolean removeItemCust(String mid, int type, short data, int amount) throws CivException {
		ArrayList<ItemInvPair> toBeRemoved = new ArrayList<ItemInvPair>();
		int count = amount;
		for (Inventory inv : invs) {
			//for (LoreMaterial loreMaterial : inv.getContents()) {
				//if(!isCorrectItemStack(LoreMaterial.materialMap.get(loreMaterial), mid, type, data)) {
				//	continue;
				//}
...
/*					toBeRemoved.add(new ItemInvPair(inv, mid, type, data, 1));
					count -= 1;
//				}
//			}
		}
...
/*	public boolean removeItemCust(LoreMaterial loreMaterial) throws CivException {
		LoreMaterial loreMat = LoreMaterial.materialMap.get(loreMaterial);
		return removeItemCust(loreMat.getId(), 0, (short)0, 1); //XXX that 1 is getAmount()
	}*/
