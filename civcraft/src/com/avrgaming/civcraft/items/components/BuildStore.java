...
import com.avrgaming.civcraft.interactive.InteractiveBuildStore;
...
public class BuildStore extends ItemComponent implements CallbackInterface {
	
	@Override
	public void onPrepareCreate(AttributeUtil attrUtil) {
		attrUtil.addLore(ChatColor.RESET+CivColor.Gold+"Build a Store");
	...
	public void buildTrommel(Player player) throws CivException {
		...
		if (!resident.hasTown()) {
			throw new CivException("You need to be in a town to build a store.");
		}
		
		if (resident.hasCamp()) {
			throw new CivException("You cannot build a trommel in a store.");
		}
		...
		info.id = "s_store";
		info.template_base_name = "store";
		info.templateYShift = 0;
		info.displayName = "Store";
		info.require_tech = "tech_commerce";
...
//		//XXX Optional, not normal
	...
	public void execute(String playerName) {
		...
		new Thread(new Runnable() {
            public void run() {
                try {
            		CivMessage.sendHeading(player, "Building a Store!");
					CivMessage.send(player, CivColor.LightGreen+ChatColor.BOLD+"Do you want to build a store here?");
					...
					resident.setInteractiveMode(new InteractiveBuildStore(resident.getTown(), player.getLocation(), null));
					ConfigBuildableInfo info = new ConfigBuildableInfo();
					Structure struct = Structure.newStructure(player.getLocation(), info.id = "s_store", resident.getTown());
					try {
