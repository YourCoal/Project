...
public class Spearman extends UnitMaterial {
	...
	public static void spawn(Inventory inv, Town town) throws CivException {
		ItemStack is = LoreMaterial.spawn(Unit.SPEARMAN_UNIT);
		...
		attrs.addLore(CivColor.LightGray+"+5% Attack with Sword");
		attrs.addLore(CivColor.LightGray+"+10% Attack with Bow");
		attrs.addLore(CivColor.LightGray+"+30% Defense on Armor");
		is = attrs.getStack();
		
		if (!Unit.addItemNoStack(inv, is)) {
			throw new CivException("Cannot make "+Unit.SPEARMAN_UNIT.getUnit().name+". Barracks chest is full! Make Room!");
		}
	}
