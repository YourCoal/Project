...
public class Swordsman extends UnitMaterial {
	...
	public static void spawn(Inventory inv, Town town) throws CivException {
		ItemStack is = LoreMaterial.spawn(Unit.SWORDSMAN_UNIT);
		...
		attrs.addLore(CivColor.LightGray+"+40% Attack with Sword");
		attrs.addLore(CivColor.LightGray+"-20% Attack with Bow");
		is = attrs.getStack();
		
		if (!Unit.addItemNoStack(inv, is)) {
			throw new CivException("Cannot make "+Unit.SWORDSMAN_UNIT.getUnit().name+". Barracks chest is full! Make Room!");
		}
	}
