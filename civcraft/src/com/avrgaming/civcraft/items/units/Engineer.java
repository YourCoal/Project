...
public class Engineer extends UnitMaterial {
	...
	public static void spawn(Inventory inv, Town town) throws CivException {
		ItemStack is = LoreMaterial.spawn(Unit.ENGINEER_UNIT);
		...
		attrs.addLore(CivColor.LightGray+"Can Build War Cannons");
		is = attrs.getStack();
		
		if (!Unit.addItemNoStack(inv, is)) {
			throw new CivException("Cannot make "+Unit.ENGINEER_UNIT.getUnit().name+". Barracks chest is full! Make Room!");
		}
	}
