...
public class Musketman extends UnitMaterial {
	...
	public static void spawn(Inventory inv, Town town) throws CivException {
		ItemStack is = LoreMaterial.spawn(Unit.MUSKETMAN_UNIT);
		...
		attrs.addLore(CivColor.LightGray+"+50% Attack with Sword");
		attrs.addLore(CivColor.LightGray+"-25% Attack with Bow");
		attrs.addLore(CivColor.LightGray+"+15% Defense on Armor");
		is = attrs.getStack();
		
		if (!Unit.addItemNoStack(inv, is)) {
			throw new CivException("Cannot make "+Unit.MUSKETMAN_UNIT.getUnit().name+". Barracks chest is full! Make Room!");
		}
	}
