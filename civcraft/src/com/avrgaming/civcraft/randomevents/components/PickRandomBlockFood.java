...
public class PickRandomBlockFood extends RandomEventComponent {
			...
			public void run() {
				try {
					...
					for (int i = 0; i < 10; i++) {
						...
						int startY = rand.nextInt(14) + 56; /* Select a y somewhere between 56 and 70. */
						for (int x = 0; x < 16; x++) {
							for (int z = 0; z < 16; z++) {
								for (int y = startY; y < 71; y++) { /* no need to check higher than 50, unlikely to find a good block. */
									Block block = coord.getChunk().getBlock(x, y, z);
									if (ItemManager.getId(block) == CivData.DIRT || ItemManager.getId(block) == CivData.GRASS ||
											ItemManager.getId(block) == CivData.LEAF || ItemManager.getId(block) == CivData.LEAF2) {
					...
					sendMessage("(Note that the cache will be in a DIRT or GRASS block, along in LEAVES on trees.)");
