...
public class BuildMenuList implements GuiAction {
	...
	public void performAction(InventoryClickEvent event, ItemStack stack) {
		...
		guiInventory = Bukkit.getServer().createInventory(player,9*2, "Structure Menu List");
		...
		if (!res.hasTown()) {
			...
			is = LoreGuiItem.build("Build Commands", ItemManager.getId(Material.COMMAND), 0, CivColor.LightGray+CivColor.ITALIC+"Select building commands.");
//			is = LoreGuiItem.setAction(is, "BuildCommandList");
			...
			if (res.getCiv().getLeaderGroup().hasMember(res) && res.getTown().isMayor(res) || res.getTown().isAssistant(res)) {
				...
				AttributeUtil attrs = new AttributeUtil(is);
					attrs.setShiny();
					is = attrs.getStack();
				guiInventory.setItem(7, is);
			} else if (!res.getCiv().getLeaderGroup().hasMember(res) && !res.getTown().isMayor(res) && !res.getTown().isAssistant(res)) {
