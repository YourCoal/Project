	...
	public void performAction(InventoryClickEvent event, ItemStack stack) {
		...
		/*if (res.getTown().hasStructure("s_library") && player.isOp()) {
			...
			if (res.getCiv().getLeaderGroup().hasMember(res) && res.getTown().isMayor(res) || res.getTown().isAssistant(res)) {
				is = LoreGuiItem.build("Structure Info", ItemManager.getId(Material.BRICK_STAIRS), 0, CivColor.LightGray+CivColor.ITALIC+"Build a structure in the town.");
				is = LoreGuiItem.setAction(is, "_1BuildStructureList");
				guiInventory.setItem(4, is);
				
				is = LoreGuiItem.build("Wonder Info", ItemManager.getId(Material.PURPUR_STAIRS), 0, CivColor.LightGray+CivColor.ITALIC+"Build a wonder in the town.");
