...
//		int offset = 0;
		...
		if (craftMat.getName() != null) {
			stack = LoreGuiItem.build("Item: "+craftMat.getName(), ItemManager.getId(Material.PAPER), 0, "");
		...
		if (!craftMat.isVanilla()) {
			stack = LoreGuiItem.build("Custom Item", ItemManager.getId(Material.EXP_BOTTLE), 0, "");
...
/*		String title = craftMat.getName()+" Recipe";
		...
		Inventory recInv = Bukkit.getServer().createInventory(player, LoreGuiItem.MAX_INV_SIZE-9, title);
		if (craftMat.isShaped()) {		
