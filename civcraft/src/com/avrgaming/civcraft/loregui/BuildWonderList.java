...
public class BuildWonderList implements GuiAction {
	...
	public void performAction(InventoryClickEvent event, ItemStack stack) {
		...
		for (ConfigBuildableInfo info : CivSettings.wonders.values()) {
			...
			ItemStack is;
			if (!res.hasTown()) {
				is = LoreGuiItem.build("Cannot Display", ItemManager.getId(Material.BEDROCK), 0, CivColor.Rose+"Must belong to a town to build a wonder.");
				...
				if (!res.getCiv().hasTechnology(info.require_tech)) {
					...
					is = LoreGuiItem.setAction(is, "BuildChooseWonderTemplate");
